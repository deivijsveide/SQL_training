...
const insertMovieGenres = (
    ...
    const genreIds = genres.map(
        genre => genreRows.find(row => row.genre === genre)!.id
    );
...
const insertMovieActors = (
    ...
    const actorIds = actors.map(
        actor => actorRows.find(row => row.full_name === actor)!.id
    );
    const values = actorIds.map(actorId => `(${movieId}, ${actorId})`).join(", ");
...
const insertMovieDirectors = (
    ...
    const directorIds = directors.map(
        director => directorRows.find(row => row.full_name === director)!.id
    );
    const values = directorIds.map(directorId => `(${movieId}, ${directorId})`).join(", ");
...
const insertMovieKeywords = (
    ...
    const keywordIds = keywords.map(
        keyword => keywordRows.find(row => row.keyword === keyword)!.id
    );
    const values = keywordIds.map(keywordId => `(${movieId}, ${keywordId})`).join(", ");
...
const insertMovieProductionCompanies = (
    ...
    const companyIds = productionCompanies.map(
        company => productionCompanyRows.find(row => row.company_name === company)!.id
    );
    const values = companyIds.map(companyId => `(${movieId}, ${companyId})`).join(", ");
