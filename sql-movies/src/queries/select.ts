export const selectActorByName = (full_name: string): string => {
  return `SELECT id, full_name FROM actors WHERE full_name = '${full_name}'`;
};
...
export const selectGenreById = (id: number): string => {
  return `SELECT id, genre FROM genres WHERE id = ${id}`;
};

export const selectDirectorById = (id: number): string => {
  return `SELECT id, full_name FROM directors WHERE id = ${id}`;
};

export const selectActorById = (id: number): string => {
  return `SELECT id, full_name FROM actors WHERE id = ${id}`;
};

export const selectKeywordById = (id: number): string => {
  return `SELECT id, keyword FROM keywords WHERE id = ${id}`;
};

export const selectProductionCompanyById = (id: number): string => {
  return `SELECT id, company_name FROM production_companies WHERE id = ${id}`;
};
...
export const selectRatingsByUserID = (userId: number) => {
